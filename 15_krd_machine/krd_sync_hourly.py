...
def setup_logging():
    ...
    # 日次ローテーションハンドラ（log_hourly_YYYYMMDD.txt形式）
    log_file = os.path.join(LOG_DIR, 'log_hourly.txt')
    ...
    handler.namer = lambda name: name.replace('log_hourly.txt.', 'log_hourly')
...
def get_all_tables(mysql_conn):
    """MySQL上の全テーブル一覧を取得（VIEWとgenpinhyo使用テーブルは除外）"""
    try:
        ...
        sql = f"""
            ...
            ORDER BY TABLE_NAME
        """
        cursor.execute(sql, [MYSQL_CONFIG['database']] + EXCLUDE_TABLES)
...
def sync_all_tables():
    """全テーブルを同期"""
    logger.info("=" * 80)
    logger.info("KRD MySQL → SQLite 低頻度同期開始（genpinhyo非使用テーブル）")
    logger.info("=" * 80)
    ...
    try:
        ...
        if table_times:
            logger.info("")
            logger.info("処理時間TOP10:")
            sorted_times = sorted(table_times, key=lambda x: x[2], reverse=True)[:10]
