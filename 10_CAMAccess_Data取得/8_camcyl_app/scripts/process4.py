...
async def process_csv_data(df, api_base_url, read_api_key, insert_api_key):
    ...
    async with httpx.AsyncClient() as client:
        ...
        for index, row in df.iterrows():
            ...
            seino = await get_seino_by_denpyono(client, api_base_url, read_api_key, denpyono)
            if not seino:
                logger.error(f"  [{row_number}/{total_rows}] ✗ エラー: SEINO取得失敗 (DENPYONO={denpyono})")
                no_seino_count += 1
                continue
...
async def main():
    ...
    result = await process_csv_data(df, api_base_url, read_api_key, insert_api_key)
