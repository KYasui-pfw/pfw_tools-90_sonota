...
01_excel_rBOM比較.pyの処理をモジュール化
- D3340/D3330/MK020/M0820からrBOMデータを取得
- Excelファイルを自動追記
- 色分け処理（水色=NOTE突合成功、黄色=MK020一致、ピンク=前工程一致、赤=取引先不一致、灰色=自動入力除外）
"""
...
def copy_and_merge_maekotei_csv(output_dir: Path):
...
def join_and_export(df_excel, df_d3340, col_h, output_dir: Path, suffix=""):
...
def update_excel_with_results(excel_path, output_dir: Path):
    ...
    for csv_path in csv_files:
        ...
        for _, row in df_csv.iterrows():
            ...
            if existing_is_numeric and existing_f_str != new_f:
                # 既存値が数値で、かつ不一致 → 赤色にして上書きしない（ただし一時的）
...
def process_december_excel(source_excel: Path, output_dir: Path):
    """12月Excelファイルの完全処理

    Args:
        source_excel: サーバーからコピーした元Excelファイル
        output_dir: 出力ディレクトリ
    ...
    """
    print("=" * 60)
    print("12月Excelファイル処理開始")
    print("=" * 60)

    output_dir.mkdir(parents=True, exist_ok=True)

    # 0. 前工程横展開CSVをコピー・縦結合
    maekotei_csv_path, _ = copy_and_merge_maekotei_csv(output_dir)

    # 1. Excelファイルをコピー
    excel_path = output_dir / "発注情報.xlsx"
    shutil.copy2(source_excel, excel_path)
    print(f"Excelファイルコピー: {source_excel} → {excel_path}")
    ...
    print("-" * 40)
    join_and_export(df_excel_both, df_d3340, col_h, output_dir, suffix="_FH両方入力済")

    # 4-B. H列のみ入力済み → D3340結合
    print("\n" + "-" * 40)
    print("【パターン2】H列入力済み × D3340")
    print("-" * 40)
    join_and_export(df_excel_h_only, df_d3340, col_h, output_dir, suffix="_H列のみ入力済")

    # 5. Excel自動追記
    output_excel = update_excel_with_results(excel_path, output_dir)
