...
#                      処理時間 目安{df1.shape[0]*4+15}秒'):
    ...
    def main():
        dt_now = datetime.now(timezone(timedelta(hours=9)))  # 日本時刻
        next_month = dt_now + relativedelta(months=1)  # 翌月
        next_next_month = dt_now + relativedelta(months=2)  # 翌翌月
        next_next_month2 = dt_now + relativedelta(months=3)  # 翌翌翌月 ADD_20241225_3か月先を出力しているケースがあったため、追加
        nen = dt_now.strftime('%Y')
        getsu = dt_now.strftime('%m')
        df1a = df_set1(nen, getsu)
        nen = next_month.strftime('%Y')
        getsu = next_month.strftime('%m')
        df1b = df_set1(nen, getsu)
        nen = next_next_month.strftime('%Y')
        getsu = next_next_month.strftime('%m')
        df1c = df_set1(nen, getsu)

        # ADD_20241225_3か月先を出力しているケースがあったため、追加
        nen = next_next_month2.strftime('%Y')
        getsu = next_next_month2.strftime('%m')
        df1d = df_set1(nen, getsu)

        #df1 = pd.concat([df1a, df1b, df1c], ignore_index=True)
        df1 = pd.concat([df1a, df1b, df1c, df1d], ignore_index=True)
