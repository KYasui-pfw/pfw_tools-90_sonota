...
try:
    ...
    def df_edit(df):
        ...
        #ADD_20250116_空欄の工程の対応（オイルミスト対応）
        combined_df = combined_df[combined_df['工程'] != '　']
        # '工程'が空文字（''）の場合に'状態'を"未完了"に更新
        combined_df.loc[combined_df['工程'] == '', '状態'] = '未完了'
        ...
        # 未完了のものはcombined_dfa、完了はcombined_dfbに分割
        # 完了はirepodbでチェックシート上に展開されている工程を利用するが、
        # 未完了のものはirepodbから取得できないため、checksheetdbから工程を展開する
        combined_dfa = combined_df[combined_df['状態'] == '未完了']
        ...
        genpin_df['工程Ver'] = genpin_df["工程Ver"].replace(
            "", 0).fillna(0)
        genpin_df['工程Ver'] = genpin_df['工程Ver'].astype(
        ...
        check_df['工程Ver'] = check_df['工程Ver'].astype(int)
        ...
        #ADD_20250116_ここでインデックスを振りなおす
        tate_df = tate_df.sort_index().sort_values(['帳票ID', '工程順'])
        tate_df = tate_df.reset_index(drop=True)
        ...
        # 帳票ID順に並べ　※工程順も入れないと順番がおかしくなる
        yoko_df = yoko_df.sort_values(['帳票ID', '工程順'])
    ...
    """
        ...
        with tab1:
            if tate_df.empty:
                ...
                with st.expander('抽出条件'):
                    ...
                    with colb3:
                        ...
                        fontsize = st.number_input(
                            f'工程文字サイズ', min_value=5, max_value=25, value=10, step=1,)
                    with colb5:
                        # 画像ファイルのパスを指定
                        st.markdown("##### 色分けの区分")
                ...
                # ADD_20250127_innnerでjoinすると、同じ行はそれぞれに結合するため、indexも含めて結合する
                # indexの行を追加
                ...
                # 工程毎の判断になるので、測定日の入っていない"状態"は未完了に変更
                tate_df.loc[tate_df['測定日'].isnull() | (
                    tate_df['測定日'] == ''), '状態'] = '未完了'

                # 分割し、未完了のdfのみ日付設定処理を行う
                tate_dfa = tate_df[tate_df['状態'] == '未完了']
                tate_dfb = tate_df[tate_df['状態'] == '完了']

                #ADD_20250116_ソートを追加（インデックス逆順）
                ...
                fig.update_layout(
                    ...
                    hovermode="closest",
                    margin=dict(l=0, r=15, t=80, b=15),
                    height=100 + 70 * len(scale_df),
        ...
        with tab2:
            ...
            if yoko_df.empty:
                ...
                # 表示の変更　判定合否
                columns_to_check = [f'判定合否{i}' for i in range(1, 11)]
                yoko_df.loc[yoko_df[columns_to_check].eq(
                    '否').any(axis=1), '状態'] = '不合格'

                # "完了"の状態を持つ行をフィルタリングし、判定合否に空欄がある場合は"着手中"に更新
                for i in range(1, 11):
                    condition = (yoko_df['状態'] == '完了') & (
                        yoko_df[f'工程{i}'] != '') & (yoko_df[f'判定合否{i}'] == '')
                    yoko_df.loc[condition, '状態'] = '着手中'

                # 未完了は未着手に変更
                yoko_df.loc[(yoko_df['状態'] == '未完了', "状態")] = '未着手'
