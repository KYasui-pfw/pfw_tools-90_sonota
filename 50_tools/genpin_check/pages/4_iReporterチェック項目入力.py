...
#                  ')
  ...
  def db_update2(edit_df):
    ...
    #df更新
    # ADD_20250123_未使用flg追加
    delete_df = edit_df[edit_df['未使用flg'] == 1]
    ...
    try:    # mdfの全データを一括でリストに変換      
      ...
      for i in range(0, len(data), batch_size):
          ...
          cur.executemany(''' UPDATE kouteizuban
                            SET 加工機１ = ?, 図番１ = ?, 加工機２ = ?, 図番２ = ?, 未使用flg = 0
                            WHERE ID = ?''', batch)
      conn.commit()
    except Exception as e:
    # エラーが発生した場合、ロールバックして変更を取り消す
    ...
    # ADD_start_20250123_未使用flg追加
    try:    # mdfの全データを一括でリストに変換      
      for _, row in delete_df.iterrows():
        cur.execute("DELETE FROM kouteizuban WHERE id = ?", (row['ID'],))
      conn.commit()
  ...
  def main_page():
    ...
    #df_kouteizuban = df_kouteizuban.drop(columns='未使用flg') #今は削除 DEL_20250123 やっぱり必要
    ...
    #df_zubancheck = df_zubancheck.drop(columns='未使用flg') #今は削除 DEL_20250123 やっぱり必要
    ...
    #with colh1:      
      ...
      df_kouteizuban_d = df_kouteizuban #ADD_20250123_未使用フラグ運用開始
      df_kouteizuban_d = df_kouteizuban_d.drop(columns='未使用flg_x') #ADD_20250123_未使用フラグ運用開始
      df_kouteizuban_d = df_kouteizuban_d.rename(columns={'未使用flg_y': '未使用flg'}) #ADD_20250123_未使用フラグ運用開始
    ...
    #各種計算処理
      ...
      merged_df4 = merged_df3[merged_df3['図番選択']==st.session_state['zumensentakuv']]
      ...
      if st.session_state['zumensentakuv'] != "" and str(zumenv) !="":
        ...
        if not merged_df4.empty:
          ...
          merged_zubancheckdf = pd.concat([df_zubancheck1,df_zubancheck2,df_zubancheck3,df_zubancheck4,df_zubancheck5,
          ...
          if btn_update:
            if nen == "" or getsu == "":
              ...
              merged_zubancheckdf = merged_zubancheckdf.drop(columns=['チェック項目', 'チェック基準'])
              edited_df2 = merged_zubancheckdf.join(edited_merged_zubancheckdf)
