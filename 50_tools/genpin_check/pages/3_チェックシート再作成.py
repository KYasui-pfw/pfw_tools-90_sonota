...
try:
    ...
    def irepo_jidoutyouhyou_create(df):
        ...
        if not err_exists:
            ...
            # CSVに変換
            csv_name = os.path.dirname(os.path.dirname(
    ...
    def df_set1(lot):
        ...
        filepath = os.path.dirname(
            os.path.dirname(__file__))+f'\\Database\\'+dbname
        db_url = f'sqlite:///{filepath}'
        ...
        with engine.connect() as connection:
            # SQLクエリの実行
            df = pd.read_sql(
                f'select * from genpinhyo where ロット番号 = "{lot}"', connection)
                ...
                if st.button("手入力現品票を印刷"):
                    ...
                    with st.spinner('現品票作成処理　実行中'):
                        ...
                        if df['ireporter管理flg'].iloc[0] == 0:  # irepo対象外
                            ...
                            df1 = df.loc[:, ['部品名', 'ロット番号', '完成部番', '月次', '組立番号', '機種', '吋', 'G', '客先名',
                                             'ﾒｯｷ', '工程VERSION', 'ireporter管理flg', 'QRコードレイアウト区分', '縦横区分', '帳票発行ID']]
                            ...
                            with st.spinner(f'iReporter帳票作成対象のため、20秒ほどお待ちください'):
                                ...
                                df4 = ireporter_data_get(sql)
                                df5 = pd.concat([df3, df4], ignore_index=True)
                                df1 = pd.merge(df1, df5, left_on=['ロット番号'], right_on=[
                                               'cluster_1_2_t'], how='left')
                        ...
                        if (df['QRコードレイアウト区分'] == 1).any():
                            # QRコード作成(旧レイアウトでの作成)
                            code = pyqrcode.create(f"{lot},{df['月次'].iloc[0]},{df['組立番号'].iloc[0]},{df['機種'].iloc[0]},{df['吋'].iloc[0]},{df['G'].iloc[0]},{
                                                   df['完成部番'].iloc[0]},{df['部品名'].iloc[0]},'',{df['客先コード'].iloc[0]},{df['ﾒｯｷ'].iloc[0]}", error='L', version=5, mode='binary')
                        else:
                            code = pyqrcode.create(f"jp.co.cimtops.ireporter.openreport:repid={
                                                   tyohyoID}", error='L', version=5, mode='binary')
                        code.png(os.path.dirname(
                            __file__)+f"\\work\\qrcode_{lot}_{dt.strftime('%H%M%S')}.png", scale=3)
                        # 余白を透明化
                        img = Image.open(os.path.dirname(
                            __file__)+f"\\work\\qrcode_{lot}_{dt.strftime('%H%M%S')}.png")
                        ...
                        for item in datas:
                            if item[0] == 255 and item[1] == 255 and item[2] == 255:
                                ...
                                newData.append(item)
                        img.putdata(newData)
                        img.save(os.path.dirname(os.path.dirname(
                            __file__))+f"\\work\\qrcode_{lot}_{dt.strftime('%H%M%S')}.png", "PNG")

                        # アウトプットのエクセルの準備
                        dt_now = datetime.now(
                            timezone(timedelta(hours=9)))  # 日本時刻
                        wb = os.path.dirname(
                            __file__)+"\\add_genpinhyo\\再発行用現品票_"+dt_now.strftime('%Y%m%d%H%M%S') + '.xlsx'
                        wb_name = "手入力現品票_" + \
                            dt_now.strftime('%Y%m%d%H%M%S') + '.xlsx'
                        shutil.copy(os.path.dirname(__file__) +
                                    '\\template\\add_template.xlsx', wb)
                        ...
                        out_wb.active.cell(3, 1).value = dt.strftime(
                            '%Y/%m/%d')  # 発行日
                        out_wb.active.cell(
                            4, 21).value = df['必要数'].iloc[0]  # 個数
                        out_wb.active.cell(
                            5, 7).value = df['完成部番'].iloc[0]  # 完成部番
                        out_wb.active.cell(
                            8, 7).value = df['B1図番'].iloc[0]  # B1部番
                        out_wb.active.cell(
                            9, 7).value = df['B2図番'].iloc[0]  # B2部番
                        out_wb.active.cell(
                            10, 7).value = df['F1図番'].iloc[0]  # F1部番
                        out_wb.active.cell(
                            11, 7).value = df['F2図番'].iloc[0]  # F2部番
                        out_wb.active.cell(
                            12, 7).value = df['L図番'].iloc[0]  # L部番
                        out_wb.active.cell(
                            13, 7).value = df['R図番'].iloc[0]  # R部番
                        out_wb.active.cell(
                            14, 7).value = df['C図番'].iloc[0]  # C部番
                        out_wb.active.cell(1, 30).value = lot  # lot番号
                        out_wb.active.cell(
                            4, 24).value = df['月次'].iloc[0]  # 月次
                        out_wb.active.cell(
                            7, 24).value = df['組立番号'].iloc[0]  # 組立番号
                        out_wb.active.cell(
                            10, 24).value = df['機種'].iloc[0]  # 機種名
                        img_to_excel = openpyxl.drawing.image.Image(os.path.dirname(os.path.dirname(
                            # QRコード
                            __file__))+f"\\work\\qrcode_{lot}_{dt.strftime('%H%M%S')}.png")
                        out_wb.active.add_image(img_to_excel, f'AI{3}')
                        out_wb.active.cell(
                            13, 32).value = df['吋'].iloc[0]  # インチ
                        out_wb.active.cell(
                            13, 38).value = df['G'].iloc[0]  # ゲージ
                        out_wb.active.cell(
                            15, 6).value = df['工程'].iloc[0]  # 工程
