  ...
  def main_page():
    ...
    #with colh2:  #★
      ...
      zumensentakuv = sl.selectbox('図番選択',zubansentaku_values,key='select1',on_change=update_zuban)
       #zumensentakuv = sl.selectbox('図番選択',zubansentaku_values,key='select1')
      ...
      #placeholderを設置で改善するか確かめる_ADD_20241128 ＞■
        ...
        if not merged_df4.empty:
          ...
          if st.button("チェック項目更新"):
            if nen == "" or getsu == "":
              ...
              #sl.empty() #■
              # ni.number_input(f'図面Ver選択　※最新：{str(int(zubanversion_values[-1]))}', min_value=1, max_value=99,value=int(zubanversion_values[-1]) ,step=1,key='innum2')
              # sb1.selectbox('図面適用生産開始年',nen_list,index=nen_list.index(dfnen),key='select3')
              # sb2.selectbox('図面適用生産計画月',tuki_list,index=tuki_list.index(str(dfgetsu)),key='select4')
              # de.data_editor(merged_zubancheckdf.drop(columns=['CID','連携ID','図面Ver','生産開始月','工程順','工程順SUB']),
              #                                     column_config={
              #                                     'チェック項目': {'width': 300,"sortable": False}, 
              #                                     'チェック基準': {'width': 300,"sortable": False}
              #                                     },hide_index=True)
