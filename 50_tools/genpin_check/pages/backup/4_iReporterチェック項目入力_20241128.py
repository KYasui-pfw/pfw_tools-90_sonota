...
# セレクトボックス1が変更されたときに他のボックスをリセットする関数
  ...
  def main_page():
    ...
    #with colh2:  #★
      ...
      #図番を選択
      zumensentakuv = st.selectbox('図番選択',zubansentaku_values,key='select1',on_change=update_zuban)
      ...
      if zumensentakuv != "" and str(zumenv) !="":
        ...
        if not merged_df4.empty:
          ...
          if st.session_state.kirikae_flg % 4 == 0:
            edited_merged_zubancheckdf = st.data_editor(merged_zubancheckdf.drop(columns=['CID','連携ID','図面Ver','生産開始月','工程順','工程順SUB']),
                                    column_config={
                                    'チェック項目': {'width': 300,"sortable": False}, 
                                    'チェック基準': {'width': 300,"sortable": False}
                                    },hide_index=True)
          elif st.session_state.kirikae_flg % 4 == 1:
            edited_merged_zubancheckdf = st.data_editor(merged_zubancheckdf.drop(columns=['CID','連携ID','図面Ver','生産開始月','工程順','工程順SUB']),
                                    column_config={
                                    'チェック項目': {'width': 300,"sortable": False}, 
                                    'チェック基準': {'width': 301,"sortable": False}
                                    },hide_index=True)
          elif st.session_state.kirikae_flg % 4 == 2:
            edited_merged_zubancheckdf = st.data_editor(merged_zubancheckdf.drop(columns=['CID','連携ID','図面Ver','生産開始月','工程順','工程順SUB']),
                                    column_config={
                                    'チェック項目': {'width': 301,"sortable": False}, 
                                    'チェック基準': {'width': 300,"sortable": False}
                                    },hide_index=True)
          elif st.session_state.kirikae_flg % 4 == 3:
            edited_merged_zubancheckdf = st.data_editor(merged_zubancheckdf.drop(columns=['CID','連携ID','図面Ver','生産開始月','工程順','工程順SUB']),
                                    column_config={
                                    'チェック項目': {'width': 301,"sortable": False}, 
                                    'チェック基準': {'width': 301,"sortable": False}
