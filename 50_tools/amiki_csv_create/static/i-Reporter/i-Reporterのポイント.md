---
marp: true
#theme: uncover
paginate: true
style: |
  h1 { font-size: 2em; }
  strong,b {color:rgb(0, 0, 0);
  background: linear-gradient(transparent 50%,rgb(255, 255, 0) 50%);}
---

<center>

# 編機調整報告書  

## i-Reporter操作のポイント

<br>
<br>
<br>
2025年5月8日  

</center>


![bg](材料/common.png)

---

<center>

## 編機調整報告書が紙から**iPad**に変わります

![alt text](image-1.png)

![bg](材料/common.png)

</center>

---

<center>

## レイアウトは紙の編機調整報告書を引き継いでいます


![w:400](image-4.png)

<style scoped>img {display: block;margin: auto;border: 1px solid #000;}</style>

![bg](材料/common.png)

</center>

---

<center>

# 使い方：iPadへのログイン


</center>

![bg](材料/common.png)


---

<center>

## iPadへのログインは端末番号で行います（小文字4桁＋数字2桁）

![w:560](image-5.png)

<style scoped>img {display: block;margin: auto;}</style>

</center>

![bg](材料/common.png)


---

### ※稀に「パスワード入力」時に入力用のキーボードが表示されないことがあります。その場合は、タブレット右上の電源ボタンを押下して画面を一旦暗くし、その後再度電源ボタンを押してから入力してください

<br>

![alt text](image-26.png)

![bg](材料/common.png)

<style scoped>img {display: block;margin: auto;}</style>

---


<center>

# 使い方：編機調整報告書の立上げ


</center>

![bg](材料/common.png)

---

<center>

## iPad運用が始まると、QRコード付きの用紙が機械に貼られます

## このQRコードをiPadの標準カメラで読み取って下さい

</center>

![w:600](image-2.png)

<style scoped>img {display: block;margin: auto;border: 1px solid #000;}</style>

![bg](材料/common.png)

---



### 目的の編機調整報告書が立ち上がるので、チェックを開始してください

<center>

![alt text](image-7.png)
<style scoped>img {display: block;margin: auto;border: 1px solid #000;}</style>

</center>

![bg](材料/common.png)

---

<center>

# 使い方：運用の注意点とお願い

</center>

![bg](材料/common.png)


---

## ①１ページ目は針並べ、２ページ目以降は調整作業の入力欄です

<center>

![alt text](image-9.png)
<style scoped>img {display: block;margin: auto;}</style>

</center>

![bg](材料/common.png)

---

### ②使用した針のLOT番号は箱のQRコードから読み取ります

### QRコードのついていない箱の場合は、手入力して下さい

![alt text](image-28.png)
<style scoped>img {display: block;margin: auto;}</style>

![bg](材料/common.png)

---

### ③作業中断時は2ページ目の右下「サーバー保存」を押下してください

### 中断後は再度QRコードを読み取ることで再開できます

<br>

![alt text](image-11.png)

<style scoped>img {display: block;margin: auto;border: 1px solid #000;}</style>

![bg](材料/common.png)

---


### 注意：一つの編機調整報告書を同時に開いて別々のチェックを行うと、後から「サーバー保存」した方が優先されるのでご注意ください

<br>

![alt text](image-25.png)

<style scoped>img {display: block;margin: auto;}</style>

![bg](材料/common.png)

---

## ④作業者名は該当の調整作業の**完了後**に、入力してください

### ※名前の入力と同時に保存処理が行われ、作業完了の実績が更新されます

<br>

<style scoped>img {display: block;margin: auto;}</style>

![h:380](image-10.png)

![bg](材料/common.png)


---

## 参考：作業実績確認画面

### ※作業の都合で順番が前後しても問題有りません

<style scoped>img {display: block;margin: auto;}</style>
![alt text](image-14.png)

![bg](材料/common.png)

---

### ⑤作業者名は初期値がニット課の社員となっています（「電気ﾁｪｯｸ」以外）

<br>
<br>

<style scoped>img {display: block;margin: auto;border: 1px solid #000;}</style>
![alt text](image-12.png)

![bg](材料/common.png)

---

### 作業者名をニット課以外から選択する場合は、

### 所属コードを空欄にして「検索」ボタンを押下してください

<style scoped>img {display: block;margin: auto;border: 1px solid #000;}</style>
![alt text](image-13.png)

![bg](材料/common.png)


---

### ⑥最終の作業者チェックが終わると、承認申請画面が立ち上がります

![alt text](image-15.png)

![bg](材料/common.png)

---

## 「承認申請」を押下した際、入力漏れがあると引っかかります


![alt text](image-16.png)
<style scoped>img {display: block;margin: auto;}</style>


![bg](材料/common.png)


---

### チェック漏れを解消し、再度承認申請を行ってください

![alt text](image-17.png)

![h:200](image-18.png)

<style scoped>img {display: block;margin: auto;}</style>

![bg](材料/common.png)

---

## 承認欄が緑のマークになれば申請完了です

### ※承認権限はGL以上が持っていますので、直接声を掛けるか、電話で連絡して最終チェックを依頼するようお願いいたします

<br>
<br>

![alt text](image-20.png)
<style scoped>img {display: block;margin: auto;}</style>

![bg](材料/common.png)

---

### 参考：承認待ち状態の進捗状況確認画面

![h:500](image-21.png)
<style scoped>img {display: block;margin: auto;}</style>

![bg](材料/common.png)

---


## ⑦電柄機、Y付、ｾﾐｼﾞｬｶﾞｰﾄﾞ機のいずれかに該当する場合は3ページ目以降が存在します

![alt text](image-22.png)


<style scoped>img {display: block;margin: auto;border: 1px solid #000;}

</style>

![bg](材料/common.png)

---

### 入力不要の項目は、「非該当」にチェックを入れてください

![alt text](image-24.png)  

<style scoped>img {display: block;margin: auto;border: 1px solid #000;}

</style>

![bg](材料/common.png)


---

<center>

# 以上です

## ありがとうございました

</center>

![bg](材料/common.png)


---

## 2025/05/09追記　以下の３点変更しました

![bg](材料/common.png)

![h:500](image-29.png)
<style scoped>img {display: block;margin: auto;border: 1px solid #000;}
</style>
