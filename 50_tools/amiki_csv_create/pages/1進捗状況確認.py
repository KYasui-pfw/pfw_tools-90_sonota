...
try:
    ...
    """
        ...
        seisan_col1, seisan_col2, seisan_col3, col_, seisan_col4, seisan_col5 = st.columns([
                                                                                     1, 1, 1, 0.3,1.3, 1])
        with seisan_col1:
        ...
        with seisan_col4:
            c_flg = 0  # 前後1次比較用
            mi1_flg = 0  # 最終チェック未完了表示flg ADD_20250516
            mi2_flg = 0  # 最終承認未完了表示flg ADD_20250516
            s_nenplus = 0
            s_nenminus = 0
            s_getsuplus = 0
            s_getsuminus = 0
            ...
            # ADD_20250516
            disop = st.selectbox(
                '＋追加表示条件', ['', '前後１次を表示', '最終ﾁｪｯｸ未完了の全表示', '最終承認未完了の全表示'])

            # ADD_20250516_チェックボックスからプルダウンに変更
            ...
            # ADD_20250516
            if disop == "最終ﾁｪｯｸ未完了の全表示":
                mi1_flg = 1
            if disop == "最終承認未完了の全表示":
                mi2_flg = 1
        ...
        # ADD_20241023分割対応
                ...
                # ADD_20250516
                if mi1_flg == 1:
                    sql += f""" or (cluster_2_165_t <> '4') OR (cluster_2_165_t IS NULL)"""
                if mi2_flg == 1:
                    sql += f""" or (cluster_2_166_t <> '4') OR (cluster_2_166_t IS NULL)"""
