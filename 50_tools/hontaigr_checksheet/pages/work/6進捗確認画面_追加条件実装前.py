...
# import streamlit as st # 重複インポート
# import pandas as pd # 重複インポート
...
def main():
    ...
    def fetch_report_data(engine, table_name, year_str, month_str, order_num_str=None):
        ...
        if month_str and month_str.startswith('0') and len(month_str) == 2:
            try:
                ...
                formatted_month_str = str(month_int)
            except ValueError:
                pass  # Keep original if not a valid number like "00"

        params = {'year': str(year_str), 'month': formatted_month_str}

        # SELECT * を使用して全カラムを取得。特定のカラム名はPython側で処理する。
        sql_query_base = f"SELECT * FROM {table_name} WHERE top_remarks1 = :year AND top_remarks2 = :month"

        if order_num_str is not None and str(order_num_str).strip() != "":
            formatted_order_num_str = order_num_str
            # e.g. "01", "02"
            if order_num_str.startswith('0') and len(order_num_str) == 2:
                try:
                    order_int = int(order_num_str)
                    formatted_order_num_str = str(
                        order_int)  # Convert to "1", "2"
                except ValueError:
                    pass  # Keep original if not a valid number
            params['order_num'] = formatted_order_num_str
            sql_query_full = sql_query_base + " AND top_remarks3 = :order_num"
        else:
    ...
    def fetch_production_data_and_merge(engine, year_str, month_str, order_str=None):
        # 生産機械情報を取得するための基本SQLクエリ
        sql_base = f'SELECT "組立番号", "年", "月", "次", "帳票No" FROM "{PRODUCTION_MACHINE_INFO_TABLE_NAME}" WHERE '
        conds, params = [], {}  # SQLクエリの条件とパラメータ

        # 年条件を追加
        try:
            conds.append('"年" = :y')
            params['y'] = int(year_str)
        except:
            st.warning("年指定が無効。")
            return pd.DataFrame()
        # 月条件を追加
        ...
        if order_str and order_str.strip():
            try:
                conds.append('"次" = :o')
                params['o'] = int(order_str)
            except:
                st.warning("「次」指定が無効。")

        if not conds:  # 条件がない場合は空のDataFrameを返す
            return pd.DataFrame()

        prod_df, report_df = pd.DataFrame(), pd.DataFrame()  # 生産および帳票タイプデータ用のDataFrame
        # 生産データを取得
        try:
            with engine.connect() as conn:
                prod_df = pd.read_sql_query(
                    text(sql_base + " AND ".join(conds)), conn, params=params)
        except Exception as e:
            st.error(f"{PRODUCTION_MACHINE_INFO_TABLE_NAME}取得エラー:{e}")
            return pd.DataFrame()

        if prod_df.empty:  # 生産データがない場合は空のDataFrameを返す
            return pd.DataFrame()
        ...
        try:
            ...
            return pd.DataFrame()  # エラーの場合は空を返すか、マージがオプションならprod_dfを返すことも可能

        if report_df.empty:
            st.warning(f"{REPORT_TYPE_TABLE_NAME}データなし。機種区分が結合できません。")
            return prod_df  # マージ用の帳票データがない場合は生産データを返す

        # 生産データと帳票データをマージ
        try:
            # マージの一貫性のために '帳票No' を文字列型に確実にする
            prod_df["帳票No"], report_df["帳票No"] = prod_df["帳票No"].astype(
                str), report_df["帳票No"].astype(str)
            merged = pd.merge(prod_df, report_df, on="帳票No", how='left')
            if "組立番号" in merged.columns:  # '組立番号' を文字列型に確実にする
                merged["組立番号"] = merged["組立番号"].astype(str)
            return merged
        except Exception as e:
            st.error(f"製造・帳票マージエラー:{e}")
            return pd.DataFrame()  # マージ失敗時もprod_dfが有用ならそれを返すことも可能
    ...
    col1, col2, col3, col4 = st.columns(4)  # 入力セレクタのレイアウト
    with col1:
        sel_y = st.selectbox(
    ...
    with col4:
        muki = st.selectbox('進捗向き', ['横', '縦'], index=0)  # ヒートマップの向き

    order_ui = None if sel_o == " " else sel_o  # 「全次数」が選択された場合はNoneを使用
    ...
    # --- データ処理と表示のループ ---
    while True:
        with placeholder.container():  # プレースホルダーを使用してコンテンツを更新
            if db_engine_postgres and db_engine_sqlite:  # DB接続が成功した場合のみ続行
                # 各帳票ビューの設定
                configs = [
                    {'n': 'view_report_636', 'c': csv_data_string_636,
                     'final_check_db_col': 'cluster_2_398_t',  # 636用に追加
                     'final_approval_db_col': 'cluster_2_397_t'},
                    {'n': 'view_report_637', 'c': csv_data_string_637,
                     'final_check_db_col': 'cluster_3_398_t',  # 637用に追加
                     'final_approval_db_col': 'cluster_3_397_t'},
                    {'n': 'view_report_638', 'c': csv_data_string_638,
                     'final_check_db_col': 'cluster_4_398_t',  # 638用に追加
                     'final_approval_db_col': 'cluster_4_397_t'}
                ]
                dfs = []  # 各設定からのDataFrameを保持するリスト
                for cfg in configs:  # 各設定を処理
                    map_df = load_base_csv_df(cfg['c'])  # CSVマッピングをロード
                    data_rows = fetch_report_data(  # PostgreSQLからデータを取得
                        db_engine_postgres, cfg['n'], sel_y, sel_m, order_ui
                    )
                    if data_rows:  # データが取得された場合
                        single_df = create_final_dataframe(  # 特別項目を含むDataFrameを作成
                            data_rows, map_df,
                ...
                if dfs:  # DataFrameが作成された場合
                    # 全てのDataFrameを連結
                    fcr_df = pd.concat(dfs, ignore_index=True)
                    fcr_df = process_final_combined_dataframe(
                        fcr_df)  # 結合されたDataFrameを処理

                    # 生産関連データ（機械情報、帳票タイプ）を取得してマージ
                    prod_rel_data = fetch_production_data_and_merge(
                        db_engine_sqlite, sel_y, sel_m, order_ui)

                    # 生産データが利用可能であればfcr_dfとマージ
                    if not prod_rel_data.empty and '組立番号' in fcr_df.columns and '組立番号' in prod_rel_data.columns:
                        # マージの一貫性のために '組立番号' を文字列型に確実にする
                        fcr_df['組立番号'], prod_rel_data['組立番号'] = fcr_df['組立番号'].astype(
                            str), prod_rel_data['組立番号'].astype(str)
                        cols_merge = ['組立番号', '機種区分']  # 生産データからマージする列
                        prod_sel = prod_rel_data[[
                            c for c in cols_merge if c in prod_rel_data.columns]]
                        if '組立番号' in prod_sel.columns:  # '組立番号' が存在すればマージ
                            fcr_df = pd.merge(fcr_df, prod_sel,
                                              on="組立番号", how="left")

                    # SQLiteからSKおよびDKタイプの進捗データを取得
                    df_sk_prog, df_dk_prog = fetch_and_merge_progress_data(
                        db_engine_sqlite, sel_y, sel_m)

                    # 「【未展開】」項目を展開し、不足しているSK/DK項目を追加するメインロジック
                    if not fcr_df.empty:
                        if 'カテゴリNo_結合結果' not in fcr_df.columns:  # 列が存在することを確認
                            fcr_df['カテゴリNo_結合結果'] = pd.NA
                        output_rows_collector = []  # 処理済み行を収集
                        newly_added_skdk_items_list = []  # 新しく追加されたSK/DK項目を収集
                        ...
                        for asm_no, group_df in fcr_df.groupby('組立番号', sort=False):
                            ...
                            group_processed_item_names = set()  # このグループで処理済みの項目名を追跡

                            for idx, original_row in group_df.iterrows():  # グループ内の行を反復処理
                                row_dict = original_row.to_dict()
                                # 項目が「【未展開】」の場合、SK/DK進捗項目で展開
                                if row_dict.get('項目名') == "【未展開】":
                                    base_data_undeployed = {k: v for k, v in row_dict.items() if k not in [
                                        '項目名', 'カテゴリNo_結合結果']}  # 展開行の基本データ
                                    # 適切な進捗DataFrame（SKまたはDK）を選択
                                    ...
                                    if target_progress_df is not None:
                                        for _, prog_row in target_progress_df.iterrows():  # 新しい行を作成
                                            new_expanded_row = {
                                                **base_data_undeployed,
                                                '項目名': prog_row['カテゴリ名'],
                                                'カテゴリNo_結合結果': prog_row['カテゴリNo_'],
                                                '作業者名': None,  # 新規項目のデフォルト
                                                '作業チェック': None,  # 新規項目のデフォルト
                                                '完了区分': 3  # 「該当なし」または「要確認」としてマーク
                                            }
                                            output_rows_collector.append(
                                                new_expanded_row)
                                            if pd.notna(new_expanded_row['項目名']):
                                                group_processed_item_names.add(  # 項目名を追跡
                                                    new_expanded_row['項目名'])
                                    else:  # 対象の進捗dfがない場合は、元の「【未展開】」行を追加
                                        output_rows_collector.append(
                                            row_dict)
                                    ...
                                    mapped_value = pd.NA  # カテゴリ番号のデフォルトマッピング値
                                    if item_name == '最終ﾁｪｯｸ_検印':
                                        mapped_value = 98  # ソート用に特定の番号を割り当て
                                    elif item_name == '最終承認':
                                        mapped_value = 99  # ソート用に特定の番号を割り当て
                                    # SK項目のカテゴリ番号をマッピング
                            ...
                            # このグループの元のデータに存在しなかったSK/DK項目を追加
                            ...
                            if target_progress_df_add is not None and 'カテゴリ名' in target_progress_df_add.columns:
                                base_info_add = {}  # 新しく追加される行の基本情報
                                if not group_df.empty:
                                    first_row = group_df.iloc[0]
                                    for col in ['組立番号', '機種区分']:  # 組立番号と機種区分をコピー
                                        if col in first_row.index:
                                            base_info_add[col] = first_row[col]

                                if '組立番号' in base_info_add:  # 組立番号が利用可能な場合のみ続行
                                    for _, prog_add_row in target_progress_df_add.iterrows():
                                        # 項目名がまだ処理されておらず、特別項目でない場合に追加
                                        if prog_add_row['カテゴリ名'] not in group_processed_item_names and prog_add_row['カテゴリ名'] not in ['最終承認', '最終ﾁｪｯｸ_検印']:
                                            new_added_row = {
                                                **base_info_add,
                                                ...
                                                # 特別項目のみが存在していた場合、新規項目は0、それ以外は3
                                            }
                                            newly_added_skdk_items_list.append(
                                                new_added_row)

                        # 処理済み行と新しく追加されたSK/DK項目を結合
                        if output_rows_collector or newly_added_skdk_items_list:
                            ...
                            if not fcr_df.empty:
                                ...
                                # ピボットテーブル表示用に項目を集約
                                # x は '組立番号' と 'カテゴリNo_結合結果_num' によるグループ
                                ...
                                if '項目名' in fcr_df.columns and '完了区分' in fcr_df.columns:
                                    fcr_df_for_pivot = fcr_df.dropna(  # ソートキーのNA行を削除
                                        subset=['カテゴリNo_結合結果_num'])
                                    pivot_prep_df = pd.DataFrame()  # ピボットテーブル準備用DataFrame
                                    if not fcr_df_for_pivot.empty:
                                        pivot_prep_df = fcr_df_for_pivot.groupby(
                                            ['組立番号', 'カテゴリNo_結合結果_num'], as_index=False).apply(aggregate_items).reset_index(drop=True)
                                    ...
                                    if not pivot_prep_df.empty and '項目名_集約' in pivot_prep_df.columns:
                                        ...
                                        if '最終ﾁｪｯｸ_検印' in pivot_prep_df.columns:  # 列が存在するか確認（空でも）
                                            final_ordered_columns.append(
                                                '最終ﾁｪｯｸ_検印')
                                    ...
                                    if '組立番号' in pivot_prep_df.columns and '項目名_集約' in pivot_prep_df.columns and '完了区分' in pivot_prep_df.columns:
                                        try:
                                            ...
                                            if not display_df.empty:  # 列を最終順序にリインデックス
                                                display_df = display_df.reindex(
                                                    columns=final_ordered_columns)
                                            else:  # 空のピボットテーブルの場合の処理
                                            ...
                                            for col in display_df.columns:
                                                # 特別項目は0でフィル
                                                if col in ['最終承認', '最終ﾁｪｯｸ_検印']:
                                                    display_df[col] = display_df[col].fillna(
                                                        0)
                                                else:  # 通常項目は3でフィル（該当なし/要確認）
                                                    display_df[col] = display_df[col].fillna(
                                                        3)
                                        except Exception as e_pivot:
                                            st.error(
                                                f"ピボットテーブル作成中にエラー: {e_pivot}")
                                            display_df = pd.DataFrame()  # エラー時は空のDataFrame
                                    else:
                                        display_df = pd.DataFrame()  # 必要な列がない場合は空
                                else:
                                    display_df = pd.DataFrame()  # 必要な列がない場合は空
                            else:  # fcr_dfが処理後に空の場合
                                fcr_df.sort_values(  # 空でなければ組立番号でソート
                                    by=['組立番号'], inplace=True, na_position='last')
                                display_df = pd.DataFrame()  # 空の表示DataFrame
                        else:  # fcr_dfが最初に空の場合
                            display_df = pd.DataFrame()

                        # fcr_dfをクリーンアップ（表示前またはピボット失敗時の最終DataFrame）
                        fcr_df.reset_index(drop=True, inplace=True)
                        fcr_df.drop_duplicates(  # 重複を削除
                            subset=['組立番号', '機種区分', '項目名'], keep='first', inplace=True)
                        fcr_df.reset_index(drop=True, inplace=True)
                    else:  # fcr_dfが最初の連結後に空の場合
                        fcr_df = pd.DataFrame(columns=fcr_df.columns if not fcr_df.empty else [  # 列が存在することを確認
                            '組立番号', '項目名', '作業者名', '作業チェック', '完了区分', '機種区分', 'カテゴリNo_結合結果'])
                        display_df = pd.DataFrame()  # 空の表示DataFrame

                    # --- ヒートマップまたはフォールバックDataFrameを表示 ---
                    if not display_df.empty:
                        dt_now_heatmap = datetime.now(
                            timezone(timedelta(hours=9)))  # タイトル用の現在時刻
                        current_ji_display = '全次'  # 次数のデフォルト表示
                        ...
                        heatmap_title = f'{sel_y}年{sel_m}月{current_ji_display}　最終更新時：{dt_now_heatmap.strftime("%Y年%m月%d日 %H時%M分")}'

                        df_heatmap_colored = display_df.copy()  # 色変換用にコピー

                        # 色マッピングのために「最終承認」と「最終ﾁｪｯｸ_検印」の値を変換（2 -> 4）
                        for special_col_name in ['最終承認', '最終ﾁｪｯｸ_検印']:
                            if special_col_name in df_heatmap_colored.columns:
                                df_heatmap_colored.loc[df_heatmap_colored[special_col_name]
                                                       == 2, special_col_name] = 4

                        z_values_processed = []  # ヒートマップ用の処理済みZ値
                        y_labels_heatmap = df_heatmap_colored.index.tolist()  # Y軸ラベル（組立番号）

                        for i, kumitate_no in enumerate(y_labels_heatmap):  # 行を反復処理
                            row_values = df_heatmap_colored.loc[kumitate_no].values
                            is_odd_row = (i % 2 == 0)  # 行の色を交互にするため
                            processed_row = []
                            for idx_col, val in enumerate(row_values):  # 列を反復処理
                                col_name_current = df_heatmap_colored.columns[idx_col]
                                if pd.isna(val):  # NaN値を処理
                                    processed_val = 3.0  # デフォルトは灰色
                                    if col_name_current in ['最終承認', '最終ﾁｪｯｸ_検印']:
                                        processed_val = 0.0  # 特別項目はデフォルトで薄水色
                                else:
                                    processed_val = float(val)

                                # 行の色を交互にするために値を調整（0と4以外）
                                if not is_odd_row:  # 偶数行の場合（0インデックスなので2行目、4行目など）
                                    if processed_val != 0.0 and processed_val != 4.0:  # 濃い青/灰色
                                        processed_val += 0.8
                                    elif processed_val == 0.0:  # 薄水色
                                        processed_val = 0.8
                                    elif processed_val == 4.0:  # 黄色
                                        processed_val = 4.8
                                processed_row.append(processed_val)
                            z_values_processed.append(processed_row)

                        z_data_for_heatmap = np.array(
                            z_values_processed)  # NumPy配列に変換
                        x_labels_heatmap = df_heatmap_colored.columns.tolist()  # X軸ラベル（項目名）

                        z_max_val = 4.8  # カラースケール正規化用の最大値
                        # 離散値のカラーマップを定義
                        color_map_strict = {
                            0.0: '#F7FbFF',  # 最も薄い青 (奇数行 - 未着手)
                            0.8: '#EBF5FF',  # 薄い青 (偶数行 - 未着手)
                            1.0: '#09306B',  # 濃い青 (奇数行 - 完了)
                            1.8: '#072A5F',  # より濃い青 (偶数行 - 完了)
                            3.0: '#898989',  # 灰色 (奇数行 - 未実施/該当なし)
                            3.8: '#7e837f',  # より濃い灰色 (偶数行 - 未実施/該当なし)
                            4.0: '#F4D800',  # 黄色 (奇数行 - 承認区分2/チェック区分2)
                            4.8: '#EEDD00',  # より濃い黄色 (偶数行 - 承認区分2/チェック区分2)
                        }

                        # Plotly用の離散カラースケールを作成
                        ...
                        for idx, key_val in enumerate(sorted_keys):
                            ...
                            if idx + 1 < len(sorted_keys):  # シャープな遷移のために次のステップを追加
                                next_key_val = sorted_keys[idx+1]
                                colorscale_heatmap_discrete.append(
                                    [(next_key_val - 0.01) / z_max_val, color_map_strict[key_val]])
                            elif key_val < z_max_val:  # 最後の色が最大まで確実に広がるようにする
                                colorscale_heatmap_discrete.append(
                                    [1.0, color_map_strict[key_val]])

                        num_A_labels = len(y_labels_heatmap)  # Yラベルの数
                        num_B_labels = len(x_labels_heatmap)  # Xラベルの数

                        # 縦向きの場合にY軸ラベルに改行を挿入するヘルパー関数
                        ...
                        # 向き（'横' または '縦'）に基づいてラベル、データ、レイアウトを調整
                        if muki == '横':  # 横向き
                            # Yラベルを反転
                            ...
                            heatmap_height = num_A_labels * 28 + 200  # 動的な高さ
                            heatmap_width = num_B_labels * 1000  # 動的な幅（調整が必要な場合あり）
                        else:  # 縦向き
                            y_labels_display = y_labels_for_display_y_axis_when_vertical
                            x_labels_display = y_labels_heatmap  # X軸に組立番号
                            z_values_display = z_data_for_heatmap.T  # Zデータを転置
                            current_xgap, current_ygap = 0, 4
                            hover_template = '組立番号: %{x}<br>項目: %{y}<br>状態値: %{z:.1f}<extra></extra>'
                            heatmap_height = num_B_labels * 27 + 200  # 動的な高さ
                            heatmap_width = num_A_labels * 1000  # 動的な幅

                        # Plotlyヒートマップフィギュアを作成
                        fig_heatmap = go.Figure(data=go.Heatmap(
                            z=z_values_display,
                            x=x_labels_display,
                            y=y_labels_display,
                            colorscale=colorscale_heatmap_discrete,
                            showscale=False,  # カラースケールバーを非表示
                            zmin=0,
                            zmax=z_max_val,
                            xgap=current_xgap,  # セル間のギャップ
                            ygap=current_ygap,
                            hovertemplate=hover_template
                        ))

                        fig_heatmap.update_layout(  # レイアウトプロパティを更新
                            title=heatmap_title,
                            autosize=False,
                            height=heatmap_height,
                            width=heatmap_width,  # より適応的にするかスクロール可能にすることを検討
                            xaxis_showgrid=False,
                            yaxis_showgrid=False,
                            margin=dict(l=0, r=0, t=30, b=10)  # マージンを調整
                        )
                        # オプション: X軸ラベルが多すぎる場合は回転
                        # if muki == '横' and len(x_labels_display) > 10:
                        #     fig_heatmap.update_xaxes(tickangle=-45)
                        # elif muki == '縦' and len(x_labels_display) > 15: # 縦向きの場合、Xラベルは組立番号
                        #     fig_heatmap.update_xaxes(tickangle=-45)

                        st.plotly_chart(fig_heatmap)  # チャートを表示
                        st.write('※５分に１度自動更新')

                    elif not fcr_df.empty:  # ピボットテーブル作成失敗したがfcr_dfにデータがある場合のフォールバック
                        st.warning(
                            "ヒートマップ表示に必要なピボットデータが作成できませんでした。加工後データを表示します。")
                        st.dataframe(fcr_df)
                    else:  # 表示するデータがない
                        st.info("指定された条件に一致する表示可能なデータがありません。")
                else:  # configsからDataFrameが作成されなかった
                    st.info("データがありません")  # 明確化のために「データがありません」から変更
            else:  # DB接続エラー
                st.error("DB接続エラー。処理を実行できません。")

        # 5分待機
        time.sleep(300)  # 自動更新間隔
