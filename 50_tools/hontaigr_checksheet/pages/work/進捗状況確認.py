    ...
    if order_num_str is not None and str(order_num_str).strip() != "":
        ...
        if order_num_str.startswith('0') and len(order_num_str) == 2:
            try:
                ...
                formatted_order_num_str = str(order_int)
            except ValueError:
                pass
...
def create_final_dataframe(db_records, column_map_df, final_approval_db_col_name=None):
    if not db_records:
        return pd.DataFrame()
    ...
    for assembly_number, group in groupby(db_records_sorted, key=lambda r: str(getattr(r, 'top_remarks4', ''))):
        ...
        for record in group_list:
            for _, csv_row_def in column_map_df.iterrows():
                ...
                try:
                    ...
                    item_value, worker_value, check_value = record_dict.get(
                        item_col_name), record_dict.get(worker_col_name), record_dict.get(check_col_name)
                    ...
                    if isinstance(item_value, str) and item_value == item_col_name:
                        item_value = None
                    worker_value = getattr(record, worker_col_name, None)
                    if isinstance(worker_value, str) and worker_value == worker_col_name:
                        worker_value = None
        ...
        if final_approval_db_col_name:
            ...
            if pd.notna(final_approval_raw_val) and str(final_approval_raw_val).strip():
                completion_status_approval = 1
                if str(final_approval_raw_val) == '2':
...
def fetch_and_merge_progress_data(engine, target_year_str, target_month_str):
    ...
    for kishu_val, target_df in [('SK', df_sk), ('DK', df_dk)]:
        ...
        try:
            with engine.connect() as conn:
                ...
                if res:
                    prog_data = res._asdict()
        except Exception as e:
            st.warning(f"{PROGRESS_CHECK_TABLE_NAME} ({kishu_val}) 取得エラー: {e}")
...
def process_final_combined_dataframe(df_orig):
    ...
    for col in ['項目名', '作業者名', '作業チェック']:
        if col in df.columns and df[col].dtype == 'object':
            ...
            df[col] = df[col].apply(robust_convert_and_clean)

    # 「最終承認」以外の項目の「作業チェック」を0に変換
    if '作業チェック' in df.columns and '項目名' in df.columns:
        df.loc[df['項目名'] != '最終承認', '作業チェック'] = df.loc[df['項目名'] != '最終承認', '作業チェック'].apply(
    ...
    if '項目名' in df.columns:
        df['項目名'] = df['項目名'].apply(lambda x: x if (pd.notna(x) and isinstance(
            x, str) and x.startswith('【') and x.endswith('】')) or x == '最終承認' else None)

    req_cols = ['組立番号', '項目名', '作業チェック']

    if not all(col in df.columns for col in req_cols):
        st.warning(f"「未展開」処理に必要な列不足。ステップ3をスキップ。")
    else:
        def apply_logic(group):
            cond1 = (group['項目名'].isnull() & group['作業チェック'].isnull()).all()
            # 「最終承認」は作業チェックが元の値なので、この条件から除外
            non_approval_group = group[group['項目名'] != '最終承認']
            cond2 = (non_approval_group['項目名'].isnull() & (
                non_approval_group['作業チェック'] == 0)).all() if not non_approval_group.empty else False
            drop = pd.Series(False, index=group.index)
            if cond1 and not group.empty:
                group.loc[group.index[0], '項目名'] = "【未展開】"
                drop.iloc[1:] = True
            elif cond2 and not group.empty:
                group.loc[group.index[0], '項目名'] = "【未展開】"
                drop.iloc[1:] = True
            else:
                # 「最終承認」以外の項目で、項目名がnullかつ作業チェックが0のものを削除
                drop = (group['項目名'].isnull()) & (
                    group['作業チェック'] == 0) & (group['項目名'] != '最終承認')
    ...
    if '項目名' in df.columns:
        is_not_final_approval = df['項目名'] != '最終承認'
        bracketed = df['項目名'].str.startswith(
            '【', na=False) & df['項目名'].str.endswith('】', na=False)
        not_undeployed = df['項目名'] != "【未展開】"

        df.loc[is_not_final_approval & bracketed & not_undeployed, '完了区分'] = 1
        df.loc[is_not_final_approval & (df['項目名'] == "【未展開】"), '完了区分'] = 0

        if '作業者名' in df.columns and '組立番号' in df.columns:
            for _, group in df.groupby('組立番号', sort=False):
                non_approval_group_completed = group[(
                    group['項目名'] != '最終承認') & (group['完了区分'] == 1)]
                for idx in non_approval_group_completed.index:
...
def fetch_production_data_and_merge(engine, year_str, month_str, order_str=None):
    ...
    prod_df, report_df = pd.DataFrame(), pd.DataFrame()
    try:
        with engine.connect() as conn:
...
orders = [" "] + [f"{o:02d}" for o in range(1, 9)]

col1, col2, col3, col4 = st.columns(4)
with col1:
    sel_y = st.selectbox("生産計画年", years, index=years.index(str(today.year)))
with col2:
    sel_m = st.selectbox("生産計画月", months, index=today.month-1)
with col3:
    sel_o = st.selectbox("生産計画次", orders, index=0)
with col4:
            ...
            for cfg in configs:
                ...
                if data_rows:
                    single_df = create_final_dataframe(
                        data_rows, map_df, cfg.get('final_approval_db_col'))
            ...
            if dfs:
                ...
                if not fcr_df.empty:
                            ...
                            if row_dict.get('項目名') == "【未展開】":
                                    ...
                                    for _, prog_row in target_progress_df.iterrows():
                                        new_expanded_row = {
                                            **base_data_undeployed,
                                            ...
                                            '作業者名': None,
                                            '作業チェック': None,
                                            '完了区分': 9,  # 修正箇所1: 「【未展開】」から展開された項目は一時的に完了区分9
                                        }
                                ...
                                if item_name == '最終承認':
                                    ...
                                    sk_map = df_sk_prog.set_index('カテゴリ名')[
                                        'カテゴリNo_']
                                    mapped_value = sk_map.get(item_name, pd.NA)
                                    ...
                                    dk_map = df_dk_prog.set_index('カテゴリ名')[
                                        'カテゴリNo_']
                                    mapped_value = dk_map.get(item_name, pd.NA)
                        ...
                        if target_progress_df_add is not None and 'カテゴリ名' in target_progress_df_add.columns:
                            ...
                            if '組立番号' in base_info_add:
                                for _, prog_add_row in target_progress_df_add.iterrows():
                                    if prog_add_row['カテゴリ名'] not in group_processed_item_names and prog_add_row['カテゴリ名'] != '最終承認':

                                        # やっと解決　長さが1の場合（最終承認のみの場合）
                                        if len(group_processed_item_names) == 1:
                                            new_added_row = {
                                                **base_info_add,
                    ...
                    if output_rows_collector or newly_added_skdk_items_list:
                        ...
                        # --- 修正箇所2 ---
                        # 「最終承認」項目は完了区分をそのまま（0, 1, or 2）とし、
                        # 「【未展開】」から来た項目（一時的に9）は0に、
                        # それ以外の新規追加項目（完了区分3）は3のままにする
                        # fcr_df.columns
                        if '項目名' in fcr_df.columns and '完了区分' in fcr_df.columns:
                            # 「最終承認」以外の項目で、完了区分が9のものを0に更新
                            fcr_df.loc[(fcr_df['項目名'] != '最終承認') & (
                                fcr_df['完了区分'] == 9), '完了区分'] = 0
                            ...
                            if 'カテゴリNo_結合結果' in fcr_df.columns:
                                ...
                                def aggregate_items(x):
                                    if '最終承認' in x['項目名'].values:
                                        ...
                                        d['カテゴリNo_結合結果_num_for_sort'] = x['カテゴリNo_結合結果_num'].iloc[0] if not x['カテゴリNo_結合結果_num'].empty else np.nan
                                    return pd.Series(d)
                                ...
                                if '項目名' in fcr_df.columns and '完了区分' in fcr_df.columns:
                                    ...
                                    if not pivot_prep_df.empty and '項目名_集約' in pivot_prep_df.columns:
                                        if 'カテゴリNo_結合結果_num_for_sort' in pivot_prep_df.columns:
                                            ...
                                            final_ordered_columns = [
                                                item for item in sorted_items_from_data if item != '最終承認']
                                            ...
                                            final_ordered_columns = [
                                                item for item in unique_items_in_data if item != '最終承認']
                                            final_ordered_columns.append(
                                                '最終承認')
                                    else:
                                        final_ordered_columns = ['最終承認']
                    ...
                    st.subheader("進捗状況ヒートマップ")
                    if not display_df.empty:
                        dt_now_heatmap = datetime.datetime.now(
                        ...
                        if '最終承認' in df_heatmap_colored.columns:
                            df_heatmap_colored.loc[df_heatmap_colored['最終承認']
                                                   == 2, '最終承認'] = 4
                        ...
                        for i, kumitate_no in enumerate(y_labels_heatmap):
                            ...
                            for idx_col, val in enumerate(row_values):
                                ...
                                if pd.isna(val):
                                    processed_val = 3.0
                                    if col_name_current == '最終承認':
                                        processed_val = 0.0
                                else:
                                    processed_val = float(val)
                                if not is_odd_row:
                                    if processed_val != 0.0 and processed_val != 4.0:  # 完了区分0と最終承認の黄色以外
                                        processed_val += 0.8
                                    elif processed_val == 0.0:  # 薄水色の場合
                                        processed_val = 0.8
                                    elif processed_val == 4.0:  # 黄色の場合
                                        processed_val = 4.8
                        ...
                        if muki == '横':
                            ...
                            y_labels_display = x_labels_heatmap
                            ...
                            heatmap_height = num_B_labels * 28 + 200
                        ...
                        elif muki == '縦' and len(x_labels_display) > 15:
                            fig_heatmap.update_xaxes(tickangle=-45)
