...
import glob
import fitz  # PyMuPDF
from PIL import Image, ImageOps # 余白トリミング関数で使用するためコメントアウトを解除
import io # 余白トリミング関数で使用するためコメントアウトを解除
...
TAB_NAMES_FOR_DISPLAY = ["EA　", "EC　", "EG　", "EI　", "EK　", "EM　", "ES　", "EW　", "発行台帳　", "配布票　"]
# 内部ロジックで使用するタブ名リスト (スペースなし)
TAB_NAMES_FOR_LOGIC = ["EA", "EC", "EG", "EI", "EK", "EM", "ES", "EW", "発行台帳", "配布票"]
...
def trim_image_whitespace_top_bottom(image_bytes: bytes, background_color_rgb=(255, 255, 255)) -> bytes:
    ...
    try:
        ...
        if top_crop <= bottom_crop : # 有効なトリミング領域があるか確認
            ...
            if cropped_image.height < 10: # あまりに小さい場合は元の画像を返す
                 return image_bytes
...
def display_pdf(pdf_path: Path):
    ...
    画質を調整し、進捗メッセージを非表示にし、画像の上下余白をトリミングする。
    """
    ...
    try:
        ...
        for page_num in range(doc.page_count):
            ...
            # 画像の上下余白をトリミングする処理を復活
            trimmed_img_bytes = trim_image_whitespace_top_bottom(img_bytes) 
            
            st.image(trimmed_img_bytes, caption=f"ページ {page_num + 1}/{doc.page_count}", use_column_width='always')
    ...
    with tabs[i]:
        if actual_tab_name == "発行台帳":
            ...
            if not model_specific_folder_parent.exists() or not model_specific_folder_parent.is_dir():
                st.error(f"基準チェック表の親フォルダ「{KIJUN_CHECK_HYO_PARENT_FOLDER_NAME}」が見つかりません。")
                continue
            
            if not model_folder.exists() or not model_folder.is_dir():
                st.warning(f"機種別フォルダ「{actual_tab_name}」が見つかりません: {model_folder}")
            ...
            if pdf_files_map:
                ...
                if session_key in st.session_state and st.session_state[session_key] in sorted_pdf_names:
                    current_selection_index = sorted_pdf_names.index(st.session_state[session_key])
                ...
                st.info(f"機種「{actual_tab_name}」のフォルダ ({model_folder.name}) には表示できるPDFファイルがありません。")
        else:
            st.error(f"不明なタブ処理です: 表示名「{tab_name_for_display}」、処理名「{actual_tab_name}」")
