...
with tab2:
    ...
    if show_search_file:
        ...
        if search_diagram_type == "廃図":
            st.markdown(f"<p style='font-size:12px; color:gray;'>廃図表示中: {selected_search_file}</p>", unsafe_allow_html=True)
        else:
            st.markdown(f"<p style='font-size:12px; color:gray;'>表示中: {selected_search_file}</p>", unsafe_allow_html=True)
    ...
    with search_info_col2:
        if search_current_structure_json.exists():
            ...
            st.caption(f"**構造ファイル更新**")
            st.caption(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(search_mod_time))}")
        else:
            st.caption(f"**構造ファイル更新**")
            st.caption("未作成")
    ...
    with search_info_col3:
        if search_current_metadata_json.exists():
            ...
            st.caption(f"**メタデータ更新**")
            st.caption(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(search_meta_mod_time))}")
        else:
            st.caption(f"**メタデータ更新**")
            st.caption("未作成")
