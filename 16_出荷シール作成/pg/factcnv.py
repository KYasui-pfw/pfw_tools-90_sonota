...
# 　変更：20250210 UKF12-0230*AA3*-5B小物のシール化
################################################################
...
def print_excel_make1(output_list, excel_name, pg_folder_path, work_folder_path):
    ...
    dt_now = datetime.datetime.now(
        datetime.timezone(datetime.timedelta(hours=9)))  # 日本時刻
    wb = work_folder_path+excel_name+"_" + \
    ...
    for i in range(list_len):
        ...
        if output_list[i][20] == 0:  # ADD_20240605_部品数7対応
            ...
            if i % 2 == 0:
                ...
                out_wb.active.merge_cells(
                    'A'+str(int(y+10))+':B'+str(int(y+10)))
                out_wb.active.merge_cells(
                    'C'+str(int(y+10))+':D'+str(int(y+10)))
                out_wb.active.merge_cells(
                    # ADD_20240605_部品数7対応
                    'A'+str(int(y+11))+':B'+str(int(y+11)))
                out_wb.active.merge_cells(
                    # ADD_20240605_部品数7対応
                    'C'+str(int(y+11))+':D'+str(int(y+11)))
            else:
                ...
                out_wb.active.merge_cells(
                    'F'+str(int(y+10))+':G'+str(int(y+10)))
                out_wb.active.merge_cells(
                    'H'+str(int(y+10))+':I'+str(int(y+10)))
                out_wb.active.merge_cells(
                    # ADD_20240605_部品数7対応
                    'F'+str(int(y+11))+':G'+str(int(y+11)))
                out_wb.active.merge_cells(
                    # ADD_20240605_部品数7対応
                    'H'+str(int(y+11))+':I'+str(int(y+11)))
            ...
            # 部品7名称 ADD_20240605_部品数7対応
            out_wb.active.cell(y+11, x+1).value = output_list[i][18]
            # 部品7数量 ADD_20240605_部品数7対応
            out_wb.active.cell(y+11, x+3).value = output_list[i][19]
        ...
        # elif output_list[i][19] == 1:  DEL_20240605_部品数7対応
            ...
            if i % 2 == 0:
                ...
                out_wb.active.merge_cells(
                    'A'+str(int(y+9))+':B'+str(int(y+10)))
                out_wb.active.merge_cells(
                    'C'+str(int(y+9))+':D'+str(int(y+10)))
            else:
                ...
                out_wb.active.merge_cells(
                    'F'+str(int(y+9))+':G'+str(int(y+10)))
                out_wb.active.merge_cells(
                    'H'+str(int(y+9))+':I'+str(int(y+10)))
...
try:
    ...
    result_df7 = pandas.DataFrame()  # ADD_20250210_GUARDNETPLATE
    ...
    for book1 in books_kousei:
        ...
        for idx in range(df3.shape[0]):
            ...
            if comp < 0:
                ...
                ##tmp_list.append(df3.iloc[idx-1, -5] +
                ##                "＋"+df3.iloc[idx-1, -6])  # ひとつ前のFを追加
                tmp_list.append(df3.iloc[idx-1, -5] + "＋" + str(df3.iloc[idx-1, -6]))
            elif comp > 0 and idx != 0:
                for idx2 in range(comp):
                    del tmp_list[-1]

            #df3.iloc[idx, -1] = str(tmp_list)+"," + \
            #    df3.iloc[idx, -5]+"＋"+df3.iloc[idx, -6]
            #df3.iloc[idx, -1] = str(tmp_list) + "," + df3.iloc[idx, -5] + "＋" + str(df3.iloc[idx, -6])

            # ▼▼▼ ここに調査用コードを追加 ▼▼▼ 20250722_エラー調査用コード
            #if not isinstance(df3.iloc[idx, -5], str):
            #    with open(errlog_dir + "調査ログ.txt", mode='a', encoding='utf-8') as f:
            #        f.write(f"--- 問題のデータを検出 ---\n")
            #        f.write(f"ファイル名: {book1}\n")
            #        f.write(f"問題のデータ（部品名称）: {df3.iloc[idx, -5]}\n")
            #        f.write(f"データ型: {type(df3.iloc[idx, -5])}\n")
            #        f.write(f"該当行の全データ:\n{df3.iloc[idx]}\n\n")
            # ▲▲▲ ここまで ▲▲▲

            df3.iloc[idx, -1] = str(tmp_list) + "," + str(df3.iloc[idx, -5]) + "＋" + str(df3.iloc[idx, -6])
            # 月次の修正
            df3.iloc[idx, 2] = "("+str(int(str(df3.iloc[idx, 2])
                                           [-3:-1]))+"-"+str(df3.iloc[idx, 2])[-1]+")"
...
# テンプレート３(スクリュー、電柄Y付チップ箱、GURDNETPLATE)[製造番号,月次,国名,製品名称1,部品補足1,部品補足2,部品1名称,部品1数量,部品2名称,部品2数量,部品3名称,部品3数量,部品4名称,部品4数量,部品5名称,部品5数量,部品6名称,部品6数量,部品7名称,部品7数量,フラグ（部品数が3以下で1）] ADD_20240605_部品数7
        ...
        df4 = df4.set_axis(
            ['製造番号', '月次', '国名', '製品名称1', '部品1名称', '部品1数量'], axis=1)

        # 部品数量を合計表示
        df4 = df4.groupby(['製造番号', '月次', '国名', '製品名称1', '部品1名称'])[
            '部品1数量'].sum().reset_index()
        ...
        if df4.loc[mask, :].shape[0] > 0:
            ...
            if log_level == 1:
                for idx in range(df4.loc[mask, :].shape[0]):
                    if df4.loc[mask, :].iloc[0, 0] == df4.loc[mask, :].iloc[idx, 0]:
                        df4err2 += '　' + \
                            str(idx+4+1)+'：' + \
                            str(df4.iloc[idx+4, 6])+'×'+str(df4.iloc[idx+4, 7])
            else:
            ...
            dt_now = datetime.datetime.now(
                datetime.timezone(datetime.timedelta(hours=9)))  # 日本時刻
            err_excel_make(df4err1, df4err2, errlog_dir,
                           dt_now.strftime('%H時%M分%S秒'))
        ...
        mask = df5['J'].str.contains(caster_list[0], na=False) & df5['J'].str.contains(
            '(?=.*ASSY. GUARD NET)', na=False)
        ...
        df5 = df5.set_axis(['製造番号', '月次', '国名', '製品名称1',
                           '部品補足1', '部品補足2', '部品1名称', '部品1数量'], axis=1)

        # 部品補足1で判定させるため、ソートをかけたうえで、上下比較
        df5 = df5.sort_values(['月次', '製造番号', '部品補足1'])

        # 部品数量を合計表示
        df5 = df5.groupby(['製造番号', '月次', '国名', '製品名称1', '部品補足1', '部品補足2', '部品1名称'])[
            '部品1数量'].sum().reset_index()
            ...
            if log_level == 1:
                for idx in range(df5.loc[mask, :].shape[0]):
                    if df5.loc[mask, :].iloc[0, 0] == df5.loc[mask, :].iloc[idx, 0]:
                        ...
                        df5err2 += '　' + \
                            str(idx+7+1)+'：' + \
                            str(df5.iloc[idx+7, 6])+'×'+str(df5.iloc[idx+7, 7])
            else:
                df5err2 += str(df5.loc[mask, '部品1名称'])
            dt_now = datetime.datetime.now(
                datetime.timezone(datetime.timedelta(hours=9)))  # 日本時刻
            err_excel_make(df5err1, df5err2, errlog_dir,
                           dt_now.strftime('%H時%M分%S秒'))

        mask = df5['部品補足1'] == df5['部品補足1'].shift(
            1)  # 部品補足1で比較する。製造番号が同じでも、2枚のシールが出ることがある。
        df5 = df5.loc[~mask, :]
        if df5.shape[0] == 1:
            df5.loc[:, '部品補足2'] = ""
        ...
        # df6.loc[mask, '部品2数量'] = 4
        # NEEDLE有り LEC SEC
        mask = df6['製品名称1'].str.contains(
            'LEC|SEC', na=False) & df6['製品名称1'].str.contains('Y', na=False)
        ...
        df7['補足1'] = ""
        df7['空欄2'] = ""
        df7 = df7.reindex(['製造番号', '月次', '分', '製品名称1', 'チェックボックス',
                          '国名', '空欄1', '補足1', '製品名称2', '空欄2'], axis=1)
        ...
        df8['空欄2'] = ""
        df8 = df8.reindex(['製造番号', '月次', '分', '製品名称1', 'チェックボックス',
                          '国名', '空欄1', '補足1', '製品名称2', '空欄2'], axis=1)
        ...
        # 部品補足のセット
        df10['部品補足1'] = "GUARD NET PLATE"  # 一旦格納
        df10['部品補足2'] = ""

        # ADD_20240529 4つ目以降の例外が発生したときようの処理
        for idx in range(len(caster_list)):
            if idx > 2:
                mask = df10['J'].str.contains(caster_list[idx], na=False)
                i = caster_list[idx].find('(')
                df10.loc[mask, '部品補足1'] = caster_list[idx][i+1:-2]
        ...
        df10 = df10.set_axis(['製造番号', '月次', '国名', '製品名称1',
                              '部品補足1', '部品補足2', '部品1名称', '部品1数量'], axis=1)
        ...
        # 部品数量を合計表示
        df10 = df10.groupby(['製造番号', '月次', '国名', '製品名称1', '部品補足1', '部品補足2', '部品1名称'])[
            '部品1数量'].sum().reset_index()
        ...
        df10['部品6数量'] = ""
        # df5['空欄'] = ""  DEL_20240605_部品数7に変更
        df10['部品7名称'] = ""  # ADD_20240605_部品数7に変更
        df10['部品7数量'] = ""  # ADD_20240605_部品数7に変更
        df10['フラグ'] = 0
        # df5 = df5.reindex(['製造番号','月次','国名','製品名称1','部品補足1','部品補足2','部品1名称','部品1数量','部品2名称','部品2数量','部品3名称','部品3数量','部品4名称','部品4数量','部品5名称','部品5数量','部品6名称','部品6数量','空欄','フラグ'], axis=1) #DEL_20240605_部品数7対応
        ...
        mask = df10['部品補足1'] == df10['部品補足1'].shift(-6)  # ADD_20240605_部品数7対応
        # ADD_20240605_部品数7対応
        ...
        # 部品が7列以上存在したとき（7個以上部品が有ったらエラー処理）
        # mask = df5['部品補足1'] == df5['部品補足1'].shift(-6) #DEL_20240605_部品数7対応
        mask = df10['部品補足1'] == df10['部品補足1'].shift(-7)  # ADD_20240605_部品数7対応
        if df10.loc[mask, :].shape[0] > 0:
            # df5err1 = "【スクリュー】7つ以上の部品が存在します" #DEL_20240605_部品数7対応
            df10err1 = "【ガードネットプレート】8つ以上の部品が存在します"  # ADD_20240605_部品数7対応
            df10err2 = str(df10.iloc[0, 0])+'の部品一覧：'
            if log_level == 1:
                for idx in range(df10.loc[mask, :].shape[0]):
                    if df10.loc[mask, :].iloc[0, 0] == df10.loc[mask, :].iloc[idx, 0]:
                        # df5err2 += '　'+str(idx+6+1)+'：'+str(df5.iloc[idx+6,6])+'×'+str(df5.iloc[idx+6,7]) #DEL_20240605_部品数7対応
                        # ADD_20240605_部品数7対応
                        df10err2 += '　' + \
                            str(idx+7+1)+'：' + \
                            str(df10.iloc[idx+7, 6])+'×' + \
                            str(df10.iloc[idx+7, 7])
            else:
                df10err2 += str(df10.loc[mask, '部品1名称'])
            dt_now = datetime.datetime.now(
                datetime.timezone(datetime.timedelta(hours=9)))  # 日本時刻
            err_excel_make(df10err1, df10err2, errlog_dir,
                           dt_now.strftime('%H時%M分%S秒'))

        mask = df10['部品補足1'] == df10['部品補足1'].shift(
            1)  # 部品補足1で比較する。製造番号が同じでも、2枚のシールが出ることがある。
        df10 = df10.loc[~mask, :]
        if df10.shape[0] == 1:
            df10.loc[:, '部品補足2'] = ""

        result_df7 = pandas.concat([result_df7, df10])
...
################################################################
    ...
    result_df7 = result_df7.sort_values(['月次', '製造番号'])  # ADD_20250210
    ...
    # ７．ガードネットプレート ADD_20250210
    # 配列化してエクセル関数に渡す
    list_result_df7 = result_df7.to_numpy().tolist()
    print_excel_make3(list_result_df7, "シール_ガードネットプレート", pg_dir, output_dir)
    ...
    if log_level == 1:
        ...
        all_df.to_excel(work_dir+"tmp\\"+back_wb_name,
                        sheet_name="中間全件データ", index=False, header=True)
        back_wb.close()

    back_wb = openpyxl.Workbook()
    back_wb_name = dt_now.strftime(
        '%Y%m%d%H%M%S')+"_ドライビングバー処理結果バックアップ"+".xlsx"
    result_df1.to_excel(work_dir+"tmp\\"+back_wb_name,
                        sheet_name="ドライビングバー", index=False, header=True)
    back_wb.close()

    back_wb = openpyxl.Workbook()
    back_wb_name = dt_now.strftime('%Y%m%d%H%M%S')+"_スクリュー処理結果バックアップ"+".xlsx"
    result_df2.to_excel(work_dir+"tmp\\"+back_wb_name,
                        sheet_name="スクリュー", index=False, header=True)
    back_wb.close()

    back_wb = openpyxl.Workbook()
    back_wb_name = dt_now.strftime(
        '%Y%m%d%H%M%S')+"_電柄 Y付きチップ箱処理結果バックアップ"+".xlsx"
    result_df3.to_excel(work_dir+"tmp\\"+back_wb_name,
                        sheet_name="電柄 Y付きチップ箱", index=False, header=True)
    back_wb.close()

    back_wb = openpyxl.Workbook()
    back_wb_name = dt_now.strftime('%Y%m%d%H%M%S')+"_KYK処理結果バックアップ"+".xlsx"
    result_df4.to_excel(work_dir+"tmp\\"+back_wb_name,
                        sheet_name="KYK", index=False, header=True)
    back_wb.close()

    back_wb = openpyxl.Workbook()
    back_wb_name = dt_now.strftime('%Y%m%d%H%M%S')+"_キャスター処理結果バックアップ"+".xlsx"
    result_df5.to_excel(work_dir+"tmp\\"+back_wb_name,
                        sheet_name="キャスター", index=False, header=True)
    back_wb.close()

    back_wb = openpyxl.Workbook()
    back_wb_name = dt_now.strftime('%Y%m%d%H%M%S')+"_ODチェック表処理結果バックアップ"+".xlsx"
    result_df6.to_excel(work_dir+"tmp\\"+back_wb_name,
                        sheet_name="ODチェック表", index=False, header=True)
    back_wb.close()

    # ADD_20250210
    back_wb = openpyxl.Workbook()
    back_wb_name = dt_now.strftime(
        '%Y%m%d%H%M%S')+"_ガードネットプレート表処理結果バックアップ"+".xlsx"
    result_df7.to_excel(work_dir+"tmp\\"+back_wb_name,
                        sheet_name="ガードネットプレート", index=False, header=True)
    back_wb.close()
