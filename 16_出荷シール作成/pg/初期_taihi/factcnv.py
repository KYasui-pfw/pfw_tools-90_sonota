...
def print_excel_make3(output_list,excel_name,pg_folder_path,work_folder_path):
  ...
  for i in range(list_len):
    ...
    out_wb.save(wb)
  #印刷範囲を指定
  out_wb.active.print_area = 'A1:I'+str(int(y+11))
...
def err_excel_make(err1,err2,err_folder_path,time,cnt=[0]):
  ...
  if wb in files:
    ...
    err_wb.active.column_dimensions["C"].width = 50
    err_wb.active.column_dimensions["D"].width = 20

    #シート内に固定文字出力（ヘッダー部）
    err_wb.active["A1"] = "部品数表示超過一覧"
    err_wb.active['A1'].font = openpyxl.styles.Font(size = 24)
    err_wb.active["A2"] = "スクリューは6種類、ドライビングバーは4種類がシールへの表示の上限です。"
    err_wb.active["A3"] = "以下のリストにシールに入りきらなかった部品があったものを記載しています。"
    err_wb.active["A4"] = "当リストが表示された場合は、対象のシールエクセルを修正していただき、手作業で追加いただきますようお願いいたします"

    err_wb.active["A6"] = "No."
    err_wb.active["B6"] = "処理時間"
    err_wb.active["C6"] = "対象"
    err_wb.active["D6"] = "製造番号"
...
################################################################
  ...
  for book1 in books_kousei: 
    ...
    for idx in range(df3.shape[0]):
      ...
      df3.iloc[idx,-1] = str(tmp_list)+","+df3.iloc[idx,-5]+","+df3.iloc[idx,-4]
...
#テンプレート３(スクリュー、電柄Y付チップ箱)[製造番号,月次,国名,製品名称1,部品補足1,部品補足2,部品1名称,部品1数量,部品2名称,部品2数量,部品3名称,部品3数量,部品4名称,部品4数量,部品5名称,部品5数量,部品6名称,部品6数量,空欄,フラグ（部品数が3以下で1）]
    ...
    if df4.loc[mask,:].shape[0] > 0:
      ...
      df4err2 = str(df4.loc[mask,'製造番号'])
      df4err2 = df4err2[4:13]
      dt_now = datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=9))) # 日本時刻      
    ...
    df6['空欄'] =""
    df6['フラグ'] =1
    df6 = df6.reindex(['製造番号','月次','国名','製品名称1','部品補足1','部品補足2','部品1名称','部品1数量','部品2名称','部品2数量','部品3名称','部品3数量','部品4名称','部品4数量','部品5名称','部品5数量','部品6名称','部品6数量','空欄','フラグ'], axis=1)
