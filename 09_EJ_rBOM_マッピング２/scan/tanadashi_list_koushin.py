...
try:
    ...
    def api_soushin(emno,alcno,lineno,tanadashi):
        
        ###データ更新処理        
        API_KEY = "uV7$flb#AtMK"
        # データを更新するエンドポイントURL
        API_URL = f"http://pfw-api/picking/"

        
        # 送信するデータ（Pythonの辞書）
        completion_data = {
            "ALCNO": str(alcno),
            "ALCLINENO": int(lineno),
            "TDDT": datetime.now().strftime('%Y-%m-%d'), #棚出日
            "IPTANCD": str(emno),
            "NOTE": "",
            "TDQTY": float(tanadashi),
            "MEINOTE": ""
        }
        # リクエストヘッダーにAPIキーを設定
    ...
    def main():
        ...
        ###### 仮設定
                # 取ってきた値をセット
        mapping_work.append({"item": "tanadashi_list", "sheet": 1, "cluster":1 , "type": "string", "value": "1"})
            ...
            #発注番号が空欄以外の場合はAPI通信
                ...
                if isinstance(data, list) and data:
                    ...
                    if data[0]["STATUS"] in ('3', '4', '8'):  # STATUS 8を完了条件に追加
                        ...
                        joutai = '登録エラー'
                    if joutai =="未登録":
                        #更新処理 引数：従業員CD、引当番号、行番号、棚出数
                        tanadashi = int(data[0]["ALCQTY"])
                        api_soushin(emno,alcno,lineno,tanadashi)
