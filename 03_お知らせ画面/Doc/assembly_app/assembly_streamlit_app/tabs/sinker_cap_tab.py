...
def render(checksheet_type: str, table_name: str):
    """
    シンカーキャップのチェックシートタブUIを描画する関数。

    Args:
        checksheet_type (str): 'シンカーキャップ'。UIのタイトルやマスターデータの取得に使用。
    ...
    with header_cols[0]:
        st.subheader("シンカーキャップ組付けチェックシート")
    with header_cols[1]:
    ...
    # --- データ取得・更新 ---
    if st.session_state.get(f"{prefix}last_fetched_year_month", None) != (selected_year, selected_month):
        with st.spinner(f"{selected_year}年{selected_month}月の{checksheet_type}生産機情報を取得・更新中..."):
            product_type = 'sinker_cap'
    ...
    instrument_names = database.get_instruments_for_checksheet(checksheet_type)[
        'instrument_no'].tolist()

    # [新規] ヘッダーに適用するカスタムCSSクラス名を定義
    CUSTOM_HEADER_CLASS = "sinker-cap-custom-header"
    ...
    instrument_column_def = {
        'width': 56,  # Assuming similar width to worker columns
        'editable': True,
        'cellEditor': 'agSelectCellEditor',
        'cellEditorParams': {'values': [""] + instrument_names},
        'cellClass': 'small-font-cell center-align',
        'headerClass': f"{CUSTOM_HEADER_CLASS} ag-header-cell-center-aligned"
    }
    date_column_def = {
    ...
    is_grid_editable = not st.session_state.get(
        f"{prefix}update_in_progress", False)
    ...
    column_defs = [
        ...
        {**worker_column_def,
            'headerName': '取付けカム(番号・種類等)', 'field': 'mounting_cam_info', 'editable': is_grid_editable},
        {**worker_column_def, 'headerName': 'カムのキズ',
            'field': 'cam_scratch', 'editable': is_grid_editable},
        {**worker_column_def, 'headerName': 'カムのエンドミルマーク',
            'field': 'cam_endmill_mark', 'editable': is_grid_editable},
        {**worker_column_def, 'headerName': 'カムの平行度',
            'field': 'cam_parallelism', 'editable': is_grid_editable},
        {**worker_column_def, 'headerName': 'カム調整',
            'field': 'cam_adjustment', 'editable': is_grid_editable},
        {**worker_column_def, 'headerName': 'カムの針通し',
            'field': 'cam_needling', 'editable': is_grid_editable},
        {**worker_column_def, 'headerName': 'キャリアリング芯出し',
            'field': 'carrier_ring_centering', 'editable': is_grid_editable},
        {**worker_column_def, 'headerName': 'キ\nャ\nリ\nア\nリ\nン\nグ\n上\n下\n0.1\n以\n内', 'field': 'carrier_ring_vertical',
            'editable': is_grid_editable, 'headerClass': 'ag-header-cell-center-aligned horizontal-text-header'},
        {**worker_column_def, 'headerName': '詰め栓(電柄Ｙ付のＳＭネジ)',
            'field': 'filler_plug', 'editable': is_grid_editable},
        {**worker_column_def, 'headerName': 'キャリアリング位置確認',
            'field': 'carrier_ring_position', 'editable': is_grid_editable},
        {**instrument_column_def, 'headerName': '計測器番号（Ｎｏ．）',
            'field': 'instrument_no', 'editable': is_grid_editable},
        {**worker_column_def, 'headerName': '予備1', 'field': 'spare_1',
            'hide': True, 'editable': is_grid_editable},
        {**worker_column_def, 'headerName': '予備2', 'field': 'spare_2',
            'hide': True, 'editable': is_grid_editable},
        {**worker_column_def, 'headerName': '予備3', 'field': 'spare_3',
            'hide': True, 'editable': is_grid_editable},
        {
            'headerName': '完成品', 'field': 'completion_status', 'width': 56,  # 75 * 0.75 = 56.25
            'editable': is_grid_editable, 'cellEditor': 'agSelectCellEditor',
            'cellEditorParams': {'values': ["", "合", "否", "不要"]},
            'cellClass': 'center-align',
            'headerClass': f"{CUSTOM_HEADER_CLASS} ag-header-cell-center-aligned"
        },
        {**date_column_def, 'headerName': '実施日',
            'field': 'completion_date', 'width': 75, 'editable': is_grid_editable},
    ...
    custom_css = {
        ...
        f".{CUSTOM_HEADER_CLASS} .ag-header-cell-label": {
            # Safari(iPad)互換性のためのプレフィックス
            "-webkit-writing-mode": "vertical-rl",
            "writing-mode": "vertical-rl",
            "text-orientation": "mixed",          # 文字の向きを明示的に指定
            "font-size": "smaller !important",
            "white-space": "pre-wrap !important",
            "line-height": "1.2em !important",
            "transform-origin": "center",         # 表示の基準点を中央に設定
            "padding-bottom": "10px",             # 下部の余白を確保して文字切れを防止
        },
        # --- ▲ここまでが修正箇所です▲ ---
        ".horizontal-text-header .ag-header-cell-label": {
