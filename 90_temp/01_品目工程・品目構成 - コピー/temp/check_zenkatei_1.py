...
def check_zenkatei_1():
    """PEFINソートの前工程=1の行と品目工程マスタの対応を確認"""
    ...
    try:
        ...
        # 前工程=1の行を抽出
        zenkatei_1_rows = pefin_df[pefin_df[zenkatei_col] == 1]
        print(f"\n前工程=1の行数: {len(zenkatei_1_rows)}行")

        if len(zenkatei_1_rows) > 0:
            print("\n前工程=1の完成部番一覧:")
            for idx, row in zenkatei_1_rows.iterrows():
            ...
            print(f"\n=== 集計結果 ===")
            print(f"前工程=1の完成部番数: {len(zenkatei_1_rows)}件")
            ...
            for idx, row in zenkatei_1_rows.iterrows():
                ...
                if len(matching_rows) > 0:
                    ...
                    if len(pefin_rows) > 0:
                        for _, pefin_row in pefin_rows.iterrows():
                            pefin_seq_analysis.append({
                                'HMCD': kansei_bango,
                                'SEQ': pefin_row['SEQ'],
                                'total_rows': len(matching_rows)
                            })

            if pefin_seq_analysis:
                print(f"\n=== PEFIN行のSEQ分析 ===")
                for item in pefin_seq_analysis:
                    print(f"{item['HMCD']}: PEFIN行のSEQ={item['SEQ']} (全{item['total_rows']}行中)")

        else:
            print("前工程=1の行が見つかりませんでした")
