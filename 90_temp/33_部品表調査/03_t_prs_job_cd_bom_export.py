"""
t_prs_job_cd_bom テーブル出力スクリプト
AWS RDS SQL Server (chohyo DB) からデータを取得
rBOM API M0810マスタとcomp_item_cdで紐づけ、HMBUNCD=7のデータを抽出
"""
import pandas as pd
...
def get_hmbuncd7_hmcd_dict() -> dict:
    """M0810からHMBUNCD='7'の品目コード(HMCD)とHMBUNCDの辞書を取得"""
    ...
    print(f"HMBUNCD=7の品目数: {len(hmcd_dict)}件")
    return hmcd_dict
...
def main():
    print("=" * 60)
    print("t_prs_job_cd_bom × M0810(HMBUNCD=7) × M0840 マッチングデータ出力")
    print("=" * 60)
    ...
    # 1. rBOM API M0810からHMBUNCD=7の品目コード辞書を取得
    hmbuncd7_hmcd_dict = get_hmbuncd7_hmcd_dict()

    if not hmbuncd7_hmcd_dict:
    ...
    # 4. comp_item_cdがHMBUNCD=7のセットに含まれるデータを抽出
    print("\n===== M0810マッチング処理 =====")
    hmbuncd7_hmcd_set = set(hmbuncd7_hmcd_dict.keys())
    df_matched = df[df['comp_item_cd'].isin(hmbuncd7_hmcd_set)].copy()
    ...
    df_matched['M0810_HMBUNCD'] = df_matched['comp_item_cd'].apply(
        lambda x: hmbuncd7_hmcd_dict.get(x, "")
    )
    ...
    output_file_full = os.path.join(work_dir, "03_t_prs_job_cd_bom_hmbuncd7.csv")
    ...
    output_file_summary = os.path.join(output_dir, "03_部品表調査サマリー.csv")
    ...
    # サマリー
    print("\n========== 処理結果サマリー ==========")
    print(f"M0810 HMBUNCD=7品目数: {len(hmbuncd7_hmcd_dict):,}件")
