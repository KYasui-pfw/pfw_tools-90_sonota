"""
D3110テーブル（製番部品表ファイル）のHMBUNCD=6品目の工程登録チェック

処理内容:
1. M0810品目マスタからHMBUNCD='6'の品目コード(HMCD)リストを取得
2. D3110からSEINO, OYALISTNO, LISTNO, HMCD, HMNM, KTCDを取得
3. HMBUNCD=6の品目を持つ行（親行）を特定
...
def get_hmbuncd6_hmcd_set() -> set:
    """M0810からHMBUNCD='6'の品目コード(HMCD)セットを取得"""
    ...
    print(f"HMBUNCD=6の品目数: {len(hmcd_set)}件")
    return hmcd_set
...
def process_data(d3110_rows: list[dict], hmbuncd6_hmcd_set: set) -> list[dict]:
    """
    HMBUNCD=6の行（親）とその子行を抽出し、工程行がない場合はKTCDに"工程登録無"を設定
    ...
    # HMBUNCD=6の親行を特定（KTCDが空欄の行のみ対象）
    parent_rows = []
    for row in d3110_rows:
        hmcd = row.get("HMCD")
        ktcd = row.get("KTCD")
        # HMBUNCD=6かつKTCDが空欄（工程行ではない）の行のみ親行として扱う
        if hmcd and hmcd in hmbuncd6_hmcd_set and not ktcd:
            parent_rows.append(row)

    print(f"HMBUNCD=6を持つ親行数（KTCD空欄のみ）: {len(parent_rows)}件")
...
def count_hmbuncd6_in_d3110(d3110_rows: list[dict], hmbuncd6_hmcd_set: set, output_dir: Path) -> dict:
    """D3110内のHMBUNCD=6品目の件数をカウントし、SEINOごとの件数をCSV出力"""
    print("\n===== HMBUNCD=6データ件数カウント =====")

    # HMBUNCD=6の品目を持つ行をカウント
    matched_rows = [row for row in d3110_rows if row.get("HMCD") in hmbuncd6_hmcd_set]
    ...
    # SEINOごとの件数をCSV出力
    seino_output_path = output_dir / "04_HMBUNCD6_SEINOごと件数.csv"
    ...
    print(f"\nHMBUNCD=6品目を含む総行数: {result['total_matched']}件")
...
def main():
    ...
    # 1. M0810からHMBUNCD=6の品目コードセットを取得
    hmbuncd6_hmcd_set = get_hmbuncd6_hmcd_set()

    if not hmbuncd6_hmcd_set:
    ...
    # 3. HMBUNCD=6データの件数カウント（SEINOごとCSV出力含む）
    count_result = count_hmbuncd6_in_d3110(d3110_rows, hmbuncd6_hmcd_set, output_dir)

    # 4. データ処理（親行と子行の抽出、工程登録無の判定）
    result_rows = process_data(d3110_rows, hmbuncd6_hmcd_set)

    if not result_rows:
        print("抽出対象データがありませんでした。")
        # カウント結果のみ表示して終了
        print("\n========== 処理結果サマリー ==========")
        print(f"HMBUNCD=6品目数（M0810マスタ）: {len(hmbuncd6_hmcd_set)}件")
        print(f"D3110総行数: {len(d3110_rows)}件")
        print(f"D3110内HMBUNCD=6マッチ行数: {count_result['total_matched']}件")
        return

    # 5. CSV出力
    output_path = output_dir / "04_HMBUNCD6工程登録チェック.csv"
