...
def main():
    ...
    """
    # ディレクトリパスの設定
    input_dir = r"C:\Dev\90_tools\90_temp\11_品目工程・品目構成（マシニング）\work"
    output_dir = r"C:\Dev\90_tools\90_temp\11_品目工程・品目構成（マシニング）\output"
    ...
    # 入力ファイルのパス
    input_file = os.path.join(input_dir, "13_品目構成work.csv")
    ...
    for index, row in df.iterrows():
        ...
        record = {
            ...
            'SHAPEQTY': 1,  # 固定で1
            'OYAQTYKBN': 1,  # 固定で1
            'OYAQTY': float(row['単位数分母']) if pd.notna(row['単位数分母']) and float(row['単位数分母']) != 0 else 1.0,  # 単位数分母を使用、0の場合は1
            'OYAUNIT': 'PC',  # 固定で"PC"
            'KOQTY': float(row['単位数分子']) if pd.notna(row['単位数分子']) and float(row['単位数分子']) != 0 else 1.0,  # 単位数分子を使用、0の場合は1
    ...
    print("KOQTY (子品目数量) ← 単位数分子（0の場合は1に変換）")
    print("OYAQTY (親品目数量) ← 単位数分母（0の場合は1に変換）")
    print("固定値: SEQ=1, STRNO=1, OYAREVNO=1, STRSEQ=1, SHAPEQTY=1")
    print("固定値: OYAQTYKBN=1, HASUKBN=1, VALFLG=1")
