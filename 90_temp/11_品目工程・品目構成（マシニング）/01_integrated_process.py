...
def main():
    # ディレクトリパスの設定
    work_dir = r"C:\Dev\90_tools\90_temp\11_品目工程・品目構成（マシニング）\work"
    output_dir = r"C:\Dev\90_tools\90_temp\11_品目工程・品目構成（マシニング）\work"

    # 出力ディレクトリが存在しない場合は作成
    os.makedirs(output_dir, exist_ok=True)

    input_dir = r"C:\Dev\90_tools\90_temp\11_品目工程・品目構成（マシニング）\input"
    ...
    if dataframes:
        ...
        print("\n=== ステップ1.5: 03_マシニング課管理工程.csv、01_購買課_対象外.csvとの突合 ===")

        # 03_マシニング課管理工程.csv、01_購買課_対象外.csv、03_購買課_そのまま.csvを読み込み
        ...
        if '完成部番' in df.columns:
            ...
            # 指示18の条件: 03_マシニング課管理工程.csvの「FINAL_ITEM_CODE」が一致する行を処理対象とする（反転）
            # 追加: 01_購買課_対象外.csvの完成部番と一致する行は対象外とする
            ...
            # 条件（マシニング課管理工程のみ反転）:
            # - 03_マシニング課管理工程.csv: 一致 → 処理対象(unmatched)
            # - 01_購買課_対象外.csv、SKD/SUJ、TS-プレフィックス、特定除外リスト: 一致 → 対象外(matched)
            exclude_mask = koubai_matched | skd_suj_mask | ts_prefix_mask | exclude_specific_mask

            # マシニング課管理工程に一致 かつ 除外条件に該当しない → 処理対象(unmatched)
            # その他 → 対象外(matched)
            unmatched_rows = data_rows[gaichu_matched & ~exclude_mask]
            matched_rows = data_rows[~(gaichu_matched & ~exclude_mask)]
            
            print(f"処理対象(unmatched)行: {len(unmatched_rows)}行")
            print(f"  - マシニング課管理工程一致: {gaichu_matched.sum()}行")
            print(f"対象外(matched)行: {len(matched_rows)}行")
...
def process_step4_ma_data(output_dir, input_filename):
    ...
    parent_dir = os.path.dirname(output_dir)  # 11_品目工程・品目構成（マシニング）ディレクトリ
